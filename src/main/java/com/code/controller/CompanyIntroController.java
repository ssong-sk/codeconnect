...
public class CompanyIntroController {
	...
	public String detail(Model model, HttpSession session) {
		...
		//내 기업 소개페이지로 이동할 때 보이는 현재의 채용공고 list

		int c_num_int = Integer.parseInt(c_num);  // String을 int로 변환


		// c_num을 통해 hire 테이블의 리스트를 가져오기
		List<HireDto> hlist = hservice.getHireListByCnum(c_num_int);
	...
	public String introshow(@RequestParam String c_num, Model model) {
		...
		//해당 기업 채용공고 리스트 보여주기
		// c_num을 가져오기
		int c_num_int = Integer.parseInt(c_num);  //c_num을 String을 int로 변환

		// c_num을 통해 hire 테이블의 리스트를 가져오기
		List<HireDto> hlist = hservice.getHireListByCnum(c_num_int);
		System.out.println(c_num_int);
